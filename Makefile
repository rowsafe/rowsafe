...
lint: check-installer
	...
	@if command -v shellcheck >/dev/null 2>&1; then \
		shellcheck -S warning -s sh scripts/install.sh scripts/test-install.sh scripts/test-rewind.sh scripts/rowsafe-agent-guard scripts/rowsafe-pg-restart scripts/rowsafe-firewall; \
