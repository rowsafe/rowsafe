...
func (a *Agent) rewindInPlace(ctx context.Context, db protocol.DatabaseSpec, p protocol.RewindInPlaceParams, tl *taskLog) (*protocol.RewindInPlaceResult, error) {
	...
	out, err := ops.restore(ctx, db, pgbackrest.RestoreOptions{DataDir: restoreDir, Type: typ, Target: target, Exclusive: typ == "xid", Set: set, Timeline: p.Target.Timeline, Repo: p.Target.Repo})
