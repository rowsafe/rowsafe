...
# upgrade keeps for undo is recorded in root's state directory.
#
...
act_pg_install_major() {
  ...
  for n in $(lsclusters | awk -v m="$major" '$1 == m { print $2 }'); do
    ...
    if pg_dropcluster --stop "$major" "$n" >>"$work_log" 2>&1; then dropped="$dropped $major/$n"; fi
  done
  add packages "$pkgs"
  add missing "$missing"
  add dropped "$dropped"
...
act_pg_upgrade() {
  ...
  record_put "$rec" "id=$id" "port=$port" "from=$c_major" "to=$major" "name=$c_name" "method=$method" \
    "aside_port=$old_port" "status=upgraded" "created=$(date +%s)" || refuse "could not record the upgrade"
  ...
  if ! timeout 600 "$systemctl" start "$new_unit" >>"$work_log" 2>&1 </dev/null; then
    ...
    if timeout 180 "$systemctl" start "$old_unit" >>"$work_log" 2>&1 </dev/null; then
      pg_dropcluster --stop "$major" "$c_name" >>"$work_log" 2>&1
      rm -f "$rec"
      add rolled_back 1
      refuse "PostgreSQL $major did not start ($why); Rowsafe put PostgreSQL $c_major back and started it"
    fi
  ...
  add new_data_dir "$(cluster_field "$major" "$c_name" 6)"
  ok=1
...
act_pg_upgrade_cleanup() {
  ...
  size=$(du -sb "$data" 2>/dev/null | cut -f1)
  : >"$work_log"
  log "removing $kept/$r_name, kept by an upgrade (request $id)"
  pg_dropcluster "$kept" "$r_name" >>"$work_log" 2>&1 || refuse "removing PostgreSQL $kept/$r_name failed: $(tail_log)"
  rm -f "$rec"
  add freed "${size:-0}"
