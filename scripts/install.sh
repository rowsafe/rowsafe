...
# connect to PostgreSQL's Unix socket. The security check edits pg_hba.conf
# when a person asks, which Debian and Ubuntu keep in /etc/postgresql (owned
# by postgres, which PostgreSQL itself can already write).
NoNewPrivileges=yes
