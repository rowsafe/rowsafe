...
# into, with everything under them): "ID files-read PATH" gives the agent
# user read access to PATH (POSIX ACLs, never ownership or modes);
# "ID files-put MODE STAGE PATH" puts files the agent restored into
# /var/lib/rowsafe/files-staging/STAGE/tree back into PATH as PATH's owner
# (read as the agent user, written as the owner; root only passes the bytes
# along). MODE: missing (never overwrite), replace, or mirror (also delete
# the files listed in STAGE/delete).
#
...
id='' action='' unit='' ok=0 err='' fpath='' fmode='' fstage=''
...
# files_put copies the staged files into the folder as its owner: the agent
# user reads them (tar), the owner writes them (tar), root only passes the
# bytes along and never opens either side.
files_put() {
  files_check_path
  ...
  if [ "$fmode" = mirror ] && as_agent test -f "$src/delete" -a ! -L "$src/delete"; then
    # shellcheck disable=SC2016 # $1 expands in the inner shell
    as_agent cat -- "$src/delete" | setpriv --reuid="$owner" --regid="$group" --clear-groups -- sh -c '
      cd -- "$1" || exit 1
  ...
  # Existing folders keep their owner and mode; missing never replaces a file.
  keep=--no-overwrite-dir
  [ "$fmode" != missing ] || keep=--skip-old-files
  flag=$(mktemp "$out_dir/.put.XXXXXX")
  out=$({ as_agent tar -C "$src/tree" -cf - . || echo read >"$flag"; } |
    setpriv --reuid="$owner" --regid="$group" --clear-groups -- tar -C "$fpath" -xpf - --no-same-owner $keep 2>&1)
  rc=$?
  if [ -s "$flag" ] || [ "$rc" != 0 ]; then
    rm -f "$flag"
...
FILES_DEFAULT_ROOTS="/srv /var/www /opt /home /data /app /var/lib/docker/volumes"
...
disallow_files() {
  ...
  if ! grep -qs '^[0-9]' "$RESTART_ALLOW_FILE"; then remove_restart_helper; fi
  if systemd_running; then systemctl daemon-reload; fi
}
