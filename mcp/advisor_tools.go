...
func (t *tools) addAdvisorTools(s *sdk.Server) {
	sdk.AddTool(s, &sdk.Tool{
		...
		Description: "Recommendations to make a database better, highest impact first, each with what to change, why, and what it costs: " +
			...
			"queries (N+1 patterns, queries reading far more than they return, results without LIMIT, sorts spilling to disk), capacity (data no longer fitting in memory, CPU saturation, connections near the limit, disk filling up, autovacuum settings for large busy tables) and indexes (unused and duplicate ones, and new indexes Rowsafe tested on a copy of the database: each with how much faster it made each query there, its size and its cost to writes). " +
