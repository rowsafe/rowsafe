...
func (t *tools) addReadTools(s *sdk.Server) {
	...
	sdk.AddTool(s, &sdk.Tool{
		Name:        "list_tasks",
		Description: "List tasks (inspect, adopt, check, backup, drill (the restore test), restore_point, restart, maintenance (a health fix a person applied), and the rewind_* tasks a person started from Rewind), newest first, for the whole organization or one database, optionally filtered by status and type. Shows who queued them (scheduler or a person), status, timing and the first line of any error. Use get_task for a task's result and log.",
