...
3. Report, worst first: what is wrong, what it puts at risk (e.g. point-in-time recovery stopped, no proven restore), the likely cause from the evidence, and the exact next step (the command or tool fleet_health gave, adjusted to what you found). When database_health lists a fix Rowsafe can apply, the next step is: click Apply fix in the Rowsafe dashboard (Pulse, Health); don't give me SQL to run instead.
4. Don't run write tools (backups, restore tests, verification, apply) during triage. Offer them, and run one only after I say yes. Rowsafe never restarts PostgreSQL on its own and you can't: restarts (Restart PostgreSQL in the dashboard, or "rowsafe restart"), health fixes (Apply fix in the dashboard) and host-side fixes are for me to do. After a restart, Rowsafe verifies by itself. If data was lost or damaged, the next step is: Rewind in the Rowsafe dashboard (restore a copy, bring the missing rows back, or rewind the whole database); rewind_window shows how far back it goes. Never restore anything yourself.`
