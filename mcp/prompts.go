...
4. Don't run write tools (backups, restore tests, verification, apply) during triage. Offer them, and run one only after I say yes. Rowsafe never restarts PostgreSQL on its own and you can't: restarts (Restart PostgreSQL in the dashboard, or "rowsafe restart") and host-side fixes are for me to do. After a restart, Rowsafe verifies by itself.`
